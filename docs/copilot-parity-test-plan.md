...
| `reach-testbed-github-go` | Trusted Copilot verification dispatcher | Codex / implemented, live proof partial | A trusted `main` workflow finds Copilot PRs and dispatches `Verify Copilot PR` without loosening public PR approval policy | Successful `Dispatch Copilot PR Verification` run that starts `Verify Copilot PR` for an `app/copilot-swe-agent` PR | Dispatcher run `27913235873` started verifier run `27913239123`; verifier succeeded and artifact `7778016209` has `verification_status=verified`; dispatcher itself failed only on best-effort PR labeling, so artifact-based skip logic was added | Push dispatcher skip fix and rerun dispatcher for a green dispatcher job |
